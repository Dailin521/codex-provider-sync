...
- Status: Accepted
- Date: 2026-08-25
- Scope: vNext C4 workspace and dependency baseline
...
根包不得运行时导入任一 workspace，不得声明 React、Vite、TypeScript 或 Electron 生产依赖；根 `files` allowlist 不包含 `apps/`、`packages/`、Fixture 或 workspace build output。当前尚未搬迁的 `web/` 源由 `apps/web` 的精确 devDependencies 构建；React 静态打包依赖也归该私有 workspace，根 manifest 不重复声明。这样 npm 8 读取单一 workspace lockfile 并执行 `npm ci --workspaces=false --omit=dev` 时，根生产树不会因 hoisted production 标记误装 React。
...
Electron、electron-vite、electron-builder、现代 UI 库和 native fallback 的具体版本不在 C4 提前解析：分别在 C5/C6/C9 按同一规则选择并记录。尤其不得为了“先搭骨架”把 Electron 写入根 manifest。
...
- 根 tarball 必须在真实临时目录安装并执行 help/status，而不是只依赖 workspace 开发环境或 pack 预览。

## Consequences

现代应用可逐步迁移到共享包而不同时搬动高风险 Core；根 CLI 仍能独立安装。代价是 C4 到 C5 期间存在一个明确记录的 `web/` 源目录和 `apps/web` 依赖所有权分离状态。该过渡只允许持续到 C5 的 Web 迁移，不能形成第二套业务实现。
...
- packlist：不存在 `apps/`、`packages/`、workspace dist、Electron 或 Fixture；
