...
test("update policy is unavailable until a supported packaged channel is configured", () => {
  ...
  assert.equal(unavailable({ isPackaged: true, platform: "darwin", arch: "arm64" }), "not-configured");
  assert.equal(unavailable({ isPackaged: true, platform: "linux", arch: "x64" }), "not-configured");
  assert.equal(unavailable({ isPackaged: true, platform: "win32", arch: "arm64" }), "unsupported-target");
  assert.equal(unavailable({ isPackaged: true, configured: true }), null);
