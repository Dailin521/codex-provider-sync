...
export interface StatusSnapshot {
  ...
  sessionActivity?: { state: "checked"; count: number } | { state: "unavailable" | "unsupported"; count: null };
  /** @deprecated Older target-scoped write-blocker metric; never use for session activity. */
  syncSessionUsage?: { state: "checked"; count: number } | { state: "unavailable" | "unsupported"; count: null };
  ...
  displayPaths?: { codexHome: string; sqliteHome: string; stateDbPath: string | null };
  backupSummary: {
    count: number;
    totalBytes: number;
  };
...
export interface HistorySessionDetail {
  ...
  storage?: { cwd: string; rolloutPath: string };
...
export type DiagnosticsTransactionState =
  | "prepared"
  | "applying"
  | "applied"
  | "skipped"
...
export interface HistoryIntegritySnapshot {
  ...
  skipped: { symlinkOrReparse: number; outOfRoot: number; notRegular: number; unreadable: number; scanLimit: number };
  displayIndex: { status: "unsupported"; reason: "no-known-display-index-schema" };
  issues: { code: string; sessionId: string | null; scope: "sessions" | "archived_sessions"; line: number | null }[];
  limits: { maxFiles: number; maxRecordsPerFile: number; maxLineBytes: number; maxIssues: number };
}
