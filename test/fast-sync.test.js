...
async function fixture(t, provider = "openai") {
  ...
  db.close();
  return { home, file, header, body, config, dbPath };
}

async function hashFrom(file, offset) {
  const hash = createHash("sha256");
  for await (const chunk of fsSync.createReadStream(file, { start: offset })) hash.update(chunk);
...
test("fast switch never opens a rollout body stream and preserves models", async (t) => {
  ...
  fsSync.createReadStream = function (file, ...args) {
    if (guarded) assert.notEqual(String(file), f.file, "fast operation must not scan the rollout body");
  ...
  // Restore v2 deliberately captures a complete pre-restore snapshot. The
  // metadata-only promise belongs to the fast sync/switch operation, not to a
  // later Restore operation.
  guarded = false;
...
test("32 MiB fast switch keeps file identity, size and rollout body bytes", async (t) => {
  const f = await fixture(t);
  const handle = await fs.open(f.file, "a");
  ...
  const bodyOffset = Buffer.byteLength(`${f.header}\n`, "utf8");
  const before = await fs.stat(f.file, { bigint: true });
  const bodyHash = await hashFrom(f.file, bodyOffset);
  const result = await runSwitch({ codexHome: f.home, provider: "prov_a", fast: true });
  ...
  assert.equal(await hashFrom(f.file, bodyOffset), bodyHash);
});
...
test("fast preflight rejects ineligible files and model intents before any mutation", async (t) => {
  for (const kind of ["length", "duplicate", "oversized", "model"]) {
    ...
    const expectedReason = {
      length: "provider-length-mismatch",
      duplicate: "provider-location-ambiguous",
      oversized: "session-meta-too-large"
    }[kind];
    await assert.rejects(runSwitch({ codexHome: f.home, provider: "prov_a", fast: true,
      ...(kind === "model" ? { model: "other" } : {}) }), (error) => {
      assert.equal(error.code, kind === "model" ? "INVALID_INPUT" : "FAST_MODE_UNSUPPORTED");
      if (expectedReason) assert.equal(error.details?.fastModeReason, expectedReason);
      return true;
    });
    assert.deepEqual(await fs.readFile(f.file), original);
...
test("CLI --fast is a flag in either position and explicitly preserves models", async (t) => {
  ...
  const json = spawnSync(process.execPath, [cli, "sync", "--provider", "prov_a", "--fast", "--json", "--codex-home", success.home], {
    encoding: "utf8", env: { ...process.env, CODEX_SQLITE_HOME: "" }
  });
  assert.equal(json.status, 0, json.stderr);
  const document = JSON.parse(json.stdout);
  assert.equal(document.ok, true);
  assert.equal(document.result.inPlaceSessionFiles, 1);
  assert.equal(document.result.rewrittenSessionFiles, 0);
  assert.equal(document.result.scanScope, "metadata");

  const unsupported = await fixture(t, "provider_old");
  const rejected = spawnSync(process.execPath, [cli, "switch", "prov_a", "--fast", "--json", "--codex-home", unsupported.home], {
    encoding: "utf8", env: { ...process.env, CODEX_SQLITE_HOME: "" }
  });
  assert.equal(rejected.status, 2, rejected.stderr);
  assert.equal(JSON.parse(rejected.stdout).error.code, "FAST_MODE_UNSUPPORTED");
});
